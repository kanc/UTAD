...
int main(int argc, char* argv[]) {
    ...
    Image* playerImg = resManager.LoadImage("data/images/isoplayer.png", 8, 8);
    IsometricMap* map = resManager.LoadIsometricMap("data/maps/isometric.tmx");
    ...
    while (screen.IsOpened()  &&  !screen.KeyPressed(GLFW_KEY_ESC)) {
        ...
        if ( !player->IsMoving() ) {
            ...
            if ( screen.KeyPressed(GLFW_KEY_LEFT) ) { toX -= scene.GetMap()->GetTileWidth(); SetDirection(player, DIRECTION_LEFT); }
            else if ( screen.KeyPressed(GLFW_KEY_RIGHT) ) { toX += scene.GetMap()->GetTileWidth(); SetDirection(player, DIRECTION_RIGHT); }
            else if ( screen.KeyPressed(GLFW_KEY_UP) ) { toY -= scene.GetMap()->GetTileHeight(); SetDirection(player, DIRECTION_UP); }
            else if ( screen.KeyPressed(GLFW_KEY_DOWN) ) { toY += scene.GetMap()->GetTileHeight(); SetDirection(player, DIRECTION_DOWN); }
