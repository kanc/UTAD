...
FILE * MyFileAccess::OpenFile(const char *filename, const char *mode)
{
...
bool MyFileAccess::CloseFile(FILE *file)
{	
	int res=0;
...
int MyFileAccess::ReadFile(FILE * file, char *buffer, int numChars)
{
	char c;
	int leidos=0;
...
int MyFileAccess::WriteFile(FILE *file, const char *buffer)
{
	char c=1;
	int carsEscritos=0;
...
int MyFileAccess::StringInFile(const char *string, FILE *file)
{
	int apariciones=0;
	int lngCadena=0;
	...
	MyFileAccess::ReadAllFile(file,textoFichero);	

	lngCadena=MyFileAccess::longuitudCadena(string);
...
void MyFileAccess::ReadAllFile(FILE *file, char *texto)
{
	char c=1;
...
int MyFileAccess::longuitudCadena(const char *cadena)
{
	int longuitud=0;
	char c;
