			...
			const std::string& imgBall, const std::string& imgBtnLeft, const std::string& imgBtnLeftPushed, const std::string& imgBtnRight, const std::string& imgBtnRightPushed )
{
	...
	bool lblEnd = m_lblMax->init(m_name + "_lblMax", Vector2(imgLeft->GetWidth() + (m_spaceControls * 2) + m_size - pFont->GetTextWidth(sMax), imgLeft->GetHeight()),font,sMax.ToCString(),255,255,255,false);	
...
void Slider::render()
{
	if (m_visible)
	{
		...
		float selectorPos = ((m_size - m_imgSelector->GetWidth()) * m_actualValue / m_maxValue); //calculamos en el que posicion se pinta la imagen del selector
		...
		Renderer::Instance().DrawTiledImage(m_imgBar, m_btnLeft->getSize().x + pos.x + m_spaceControls, pos.y + (m_btnLeft->getSize().y / 2) - (m_imgBar->GetHeight() / 2), m_size ,m_imgBar->GetHeight());	
